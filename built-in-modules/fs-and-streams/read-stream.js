...
// fs.readFile() waits until the entire file is read before invoking the callback function. If we are reading a huge file, it is going to create some latency. So, a better solution might be to use a readable stream
...
// what makes streams unique, is that instead of a program reading a file into memory all at once, streams read chunks of data piece by piece, processing its content without keeping it all in memory
...
const stream = fs.createReadStream(
  './lib/chat-logs/george-ben-chat.log',

  { highWaterMark: 999, encoding: 'utf8' }
);
